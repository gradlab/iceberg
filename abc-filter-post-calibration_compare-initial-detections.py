...
out_folder2 = 'outputs/sim_results_compare_initial_detections/'
...
for abc_par_iter in np.arange(accepted.shape[0]):
    for group_iter, group_name in enumerate(group_names_short):
        ...
        fname = out_folder2+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_initial-detections'+ str(1)+'.csv'    
        if os.path.isfile(fname)==False:
            # read data
            combodf = pd.read_csv(out_folder+'simulation_data_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'.csv')

            # reshape r detection data as prep for filtering
            ...
            for initial_detections in np.arange(1,max_initial_detections):
                ...
                fname = out_folder2+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_initial-detections'+ str(initial_detections)+'.csv'    
                if os.path.isfile(fname)==False:
                    ...
                    combodf_all_agg.to_csv(out_folder2+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_initial-detections'+ str(initial_detections)+'.csv', index=False)
...
for initial_detections in np.arange(1,max_initial_detections):
    ...
    for group_iter, group_name in enumerate(group_names_short):
        ...
        # check if any of the abc parameter - group iter combo exists
        fname = out_folder2+'elimdf_agg_intro-group-'+group_name+'_initial-detections'+ str(initial_detections)+'.csv'
        if os.path.isfile(fname)==False:
            ...
            for abc_par_iter in np.arange(accepted.shape[0]):
                ...
                combodf_all_agg = pd.read_csv(out_folder2+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_initial-detections'+ str(initial_detections)+'.csv')
                ...
                for ii in np.arange(combodf_all_agg.iteration.max()):
                    ...
                    for n_zeros in set_of_zeros:
                        ...
                        if sub['ABC_iteration_filter_'+str(n_zeros)].max()>0:
                            ...
                            # diff between cum cases at det time and 60 days later
            ...
            # save 
            #elimdf_all.to_csv(out_folder2+'elimdf_all_intro-group-'+group_name++'_initial-detections'+ str(initial_detections)+'.csv', index=False)
            
            # aggregate over iterations
            elimdf_agg = elimdf_all.groupby(['intro_group','abc_par_iter', 'n_zeros']).agg('mean').reset_index()
            # save
            elimdf_agg.to_csv(out_folder2+'elimdf_agg_intro-group-'+group_name+'_initial-detections'+ str(initial_detections)+'.csv', index=False)
