...
out_folder = 'outputs/sim_results_varying_treatment_failure/'
...
for treat_fail in [0.3, 0.5, 0.7, 0.9]:
    ...
    for abc_par_iter in np.arange(accepted.shape[0]):
        ...
        for group_iter, group_name in enumerate(group_names_short):
            ...
            fname = out_folder+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_treat-fail-'+ str(int(treat_fail*10))+'.csv'
            if os.path.isfile(fname)==False:
            
                # read data
                combodf = pd.read_csv(out_folder+'simulation_data_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_treat-fail-'+ str(int(treat_fail*10))+'.csv')
                ...
                combodf_all_agg.to_csv(out_folder+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_treat-fail-'+ str(int(treat_fail*10))+'.csv', index=False)
...
for treat_fail in [0.3, 0.5, 0.7, 0.9]:
        ...
        for group_iter, group_name in enumerate(group_names_short):
            ...
            # check if any of the abc parameter - group iter combo exists
            fname = out_folder+'elimdf_agg_intro-group-'+group_name+'_treat-fail-'+ str(int(treat_fail*10))+'.csv'
            if os.path.isfile(fname)==False:
                ...
                for abc_par_iter in np.arange(accepted.shape[0]):
                    ...
                    combodf_all_agg = pd.read_csv(out_folder+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_treat-fail-'+ str(int(treat_fail*10))+'.csv')
                ...
                # save
                elimdf_agg.to_csv(out_folder+'elimdf_agg_intro-group-'+group_name+'_treat-fail-'+ str(int(treat_fail*10))+'.csv', index=False)
