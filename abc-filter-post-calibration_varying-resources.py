...
out_folder = 'outputs/sim_results_varying_resources/'
out_folder2 = 'outputs/sim_results_varying_resources/'
...
# 2 initial detections
initial_detections = 1

# read accepted parameters
...
# rates of antibiotic susceptibility testing
for ast_rate in [0.01, 0.05, 0.1, 0.2, 0.3, 0.4]:
    
    # multipliers for screening
    for screen_multiplier in [0.5, 1., 1.5, 2.]:
        ...
        for abc_par_iter in np.arange(accepted.shape[0]):
            ...
            for group_iter, group_name in enumerate(group_names_short):
                ...
                # read data
                combodf = pd.read_csv(out_folder+'simulation_data_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_detection-percent-'+str(int(ast_rate*100))+'_screening-multiplier-'+str(screen_multiplier)+'.csv')
                ...
                # save
                # combodf_all.to_csv(out_folder2+'simulation_data_with_abc_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_initial-detections'+ str(initial_detections)+'.csv', index=False)
                combodf_all_agg.to_csv(out_folder2+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_detection-percent-'+str(int(ast_rate*100))+'_screening-multiplier-'+str(screen_multiplier)+'.csv', index=False)
...
for ast_rate in [0.01, 0.05, 0.1, 0.2, 0.3, 0.4]:
    ...
    for screen_multiplier in [0.5, 1., 1.5, 2.]:
        ...
        for group_iter, group_name in enumerate(group_names_short):
            ...
            fname = out_folder2+'elimdf_agg_intro-group-'+group_name+'_detection-percent-'+str(int(ast_rate*100))+'_screening-multiplier-'+str(screen_multiplier)+'.csv'
            if os.path.isfile(fname)==False:
                ...
                for abc_par_iter in np.arange(accepted.shape[0]):
                    ...
                    combodf_all_agg = pd.read_csv(out_folder2+'simulation_data_with_abc_agg_index-group-'+group_name+'_abc-params-'+str(abc_par_iter)+'_detection-percent-'+str(int(ast_rate*100))+'_screening-multiplier-'+str(screen_multiplier)+'.csv')
                    ...
                    for ii in np.arange(combodf_all_agg.iteration.max()):
                        ...
                        for n_zeros in set_of_zeros:
                            ...
                            if sub['ABC_iteration_filter_'+str(n_zeros)].max()>0:
                                ...
                                # note: no new cases at the END of the zeros
                ...
                # save
                elimdf_agg.to_csv(out_folder2+'elimdf_agg_intro-group-'+group_name+'_detection-percent-'+str(int(ast_rate*100))+'_screening-multiplier-'+str(screen_multiplier)+'.csv', index=False)
